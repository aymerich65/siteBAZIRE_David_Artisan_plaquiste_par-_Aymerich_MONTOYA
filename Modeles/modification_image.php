...
if (isset($_FILES['image']) && $_FILES['image']['error'] === UPLOAD_ERR_OK) {
    ...
    $query = $pdo->prepare("SELECT filename FROM galerie_images WHERE id = :id");
    $query->bindValue(':id', $id, PDO::PARAM_INT);
    $query->execute();
    $result = $query->fetch(PDO::FETCH_ASSOC);
    $oldFilename = $result['filename'];
    $oldFilePath = '../images/' . $oldFilename;
    if (file_exists($oldFilePath)) {
        unlink($oldFilePath);
    }

    // Suite du traitement de l'image comme dans le script précédent
